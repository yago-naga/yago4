...
public class PairStreamSetAntiJoinSpliterator<K, V> extends AbstractStreamSpliterator<Map.Entry<K, V>, Map.Entry<K, V>> {

  private final Set<K> right;

  public PairStreamSetAntiJoinSpliterator(Spliterator<Map.Entry<K, V>> left, Set<K> right) {
    super(left);
    this.right = right;
  }
  ...
  protected Spliterator<Map.Entry<K, V>> buildSplit(Spliterator<Map.Entry<K, V>> parentSplit) {
    return new PairStreamSetAntiJoinSpliterator<>(parentSplit, right);
  }
}
